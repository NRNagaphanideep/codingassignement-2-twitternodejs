...
app.post("/register/", async (request, response) => {
  ...
  const userCheckingInDatabase = `SELECT * FROM user WHERE
username = '${username}';`;
...
const getUserIdFun = async (username) => {
  const userIdQuery = `SELECT user_id FROM user WHERE username
= '${username}'`;
...
follower.following_user_id=tweet.user_id) AS T NATURAL JOIN user
WHERE
...
SELECT username AS name FROM user INNER JOIN follower ON
...
*
FROM
tweet INNER JOIN follower ON tweet.user_id =
...
username
FROM
(tweet INNER JOIN like ON tweet.tweet_id = like.tweet_id)INNER
...
reply
FROM
(tweet INNER JOIN reply ON tweet.tweet_id = reply.tweet_id ) INNER
